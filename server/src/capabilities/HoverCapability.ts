...
export class HoverCapability extends AbstractCapability {
	...
	protected getMarkdownByNode(foundNodeByLine: LinePositionedNode<AbstractNode>, parsedFile: ParsedFusionFile, workspace: FusionWorkspace) {
		...
		if (node instanceof PrototypePathSegment) return this.getMarkdownForPrototypeName(workspace, <FusionObjectValue | PrototypePathSegment>node, parsedFile)
			...
			for (const otherPositionedNode of [...otherParsedFile.prototypeCreations, ...otherParsedFile.prototypeOverwrites]) {
				...
				if (prototypeNode["identifier"] === prototypeName) {
					...
					if (foundIgnoreComment?.getNode().multiline && foundIgnoreComment?.getNode().value.startsWith("*\n")) {
						...
						const commentLines = lines.splice(1, lines.length - 2).map(line => line.replace(" * ", ""))
